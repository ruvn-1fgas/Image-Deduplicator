...
void DuplWindow::AddStrings()
{
    ...
    gtk_grid_attach(GTK_GRID(main_grid_), scroll, 0, 2, 3, 1);
}

void DuplWindow::OnPrevButtonClicked(GtkWidget *button)
{
...
void DuplWindow::OnNextButtonClicked(GtkWidget *button)
{
    GdkDevice *device = gdk_seat_get_pointer(gdk_display_get_default_seat(gdk_display_get_default()));
    GdkModifierType state = gdk_device_get_modifier_state(device);
...
void DuplWindow::OnListBoxRowActivated(GtkListBox *box, GtkListBoxRow *row)
{
...
void DuplWindow::OnListBoxRightClickPressed(GtkGesture *gesture, gint n_press, gdouble x, gdouble y)
{
    ...
    g_signal_connect(open_all, "clicked", G_CALLBACK(OpenAllFilesCallback), this);
    g_signal_connect(delete_all, "clicked", G_CALLBACK(DeleteAllFilesCallback), this);
...
void DuplWindow::OnListBoxLabelQueryTooltip(GtkWidget *widget, gint x, gint y, gboolean keyboard_mode, GtkTooltip *tooltip)
{
...
void DuplWindow::OpenAllFiles(GSimpleAction *action, GVariant *parameter)
{
    GList *selected_rows = gtk_list_box_get_selected_rows((GtkListBox *)list_box_);
...
void DuplWindow::DeleteAllFiles(GSimpleAction *action, GVariant *parameter)
{
    GList *selected_rows = gtk_list_box_get_selected_rows((GtkListBox *)list_box_);
